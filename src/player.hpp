...
struct Player
{
    ...
    const float ACCEL           = 8.0f;     // movement acceleration.
    const float MAX_SPEED       = 15.0f;    // maxiumum player speed.
    const float ANIM_SPEED      = 1.2f;     // animation speed.
    const float TURN_SPEED      = 10.0f;    // speed of model rotation.
    const float JUMP_POWER      = 60.0f;    // jump impulse amount.
    const float JUMP_BOOST      = 30.0f;    // increased jump from jump pad.
    const float GRAVITY         = 130.0f;   // strength of gravity.
    ...
    float jump_hold             = 0.4f;
    float jump_buffer           = 0.1f;
    ...
    void move(glm::vec3 movement, std::unique_ptr<Level> &level, Skybox &skybox, Sound &bgm, float dt);
    void respawn(std::unique_ptr<Level> &level, Skybox &skybox, Sound &bgm, float dt);
    void jump(float power, float dt);

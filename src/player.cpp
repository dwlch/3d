...
Player::Player()
{
    // 2 colliders, one for EPA that is solid, the other that is just for getting grounded state by extending slightly from base of main collider.
...
void Player::update(float delta_time, std::vector<std::unique_ptr<Collider>> &colliders, Camera &camera)
{
    ...
    if (!is_vec2_zero(input))
    {
        ...
        model.update_animations(delta_time);
    }

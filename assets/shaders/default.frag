...
{
    float shadow = 0.0;
    for (int i = 0; i < 1; i++) // this bit needs fixing; make it draw all cascades properly.
    {
        vec3 shadow_coords  = ((fragPosLight[i].xyz / fragPosLight[i].w) + 1.0) / 2.0;
        ...
        shadow = bilinear_pcf(shadow_coords, i);
    }
